...
class Transformer(object):
    ...
    def get_one_reward_baseline(self, origin_inputs, gen_targets, roll_num, dis_model):
        baseline = dis_model.build_bleu_discriminator(origin_inputs=origin_inputs,
        ...
        for i in range(roll_num):
            ...
            print("roll_sample", roll_sample)
            cur_reward = dis_model.build_bleu_discriminator(
                origin_inputs=origin_inputs,
                gen_targets=roll_sample
            )   # [batch ,1]
            total_rewards.append(cur_reward) # list, [batch,1] * roll_num
    ...
    def get_one_reward_loss(self, origin_inputs, gen_targets, origin_targets, roll_num, dis_model, margin=1.0):
        real_loss = dis_model.get_real_loss(origin_inputs=origin_inputs,
                                            origin_target=origin_targets)
        tf.identity(real_loss[:5], "real_loss")
        bs_fake_loss = dis_model.get_fake_loss(origin_inputs=origin_inputs,
                                               gen_targets=gen_targets)
        tf.identity(bs_fake_loss[:5], "bs_fake_loss")
        baseline = 1 / tf.maximum(1.0, bs_fake_loss / real_loss)
        max_len = tf.shape(gen_targets)[1]
        lengths = tf.reduce_sum(tf.cast(tf.not_equal(gen_targets, PAD_ID), tf.int32), axis=1)
        min_len = lengths[tf.argmin(lengths)]
        gen_targets_mask = tf.cast(tf.not_equal(gen_targets, PAD_ID), tf.float32)
        ...
        for i in range(roll_num):
            ...
            print("roll_sample", roll_sample)
            fake_loss = dis_model.get_fake_loss(
                origin_inputs=origin_inputs,
                gen_targets=roll_sample
            )   # [batch ,1]
            tf.identity(fake_loss[:5], "fake_loss")
            cur_reward = 1 / tf.maximum(1.0, fake_loss / real_loss)  # [batch]
            print("cur_reward", cur_reward)
            total_rewards.append(tf.reshape(cur_reward, [-1,1])) # list, [batch,1] * roll_num
        total_rewards = tf.reduce_mean(tf.concat(total_rewards, axis=1), axis=1) # [bacth, roll_num] -> [batch, ]
        tf.identity(total_rewards[:5], "mean_rewards")
        total_rewards = tf.maximum(0.0, total_rewards - tf.reshape(baseline, [-1]))
        return given_num, total_rewards
    ...
    #        fake_prediction = self.argmax_predict(encoder_outputs, attention_bias)
...
class Discriminator(Transformer):
    def __init__(self, is_train, params):
        super(Discriminator, self).__init__(is_train, params)
...
if __name__ == "__main__":
    ...
    # print(roll_samples)
    given_num, total_rewards = model.get_one_reward_loss(origin_inputs=x_inputs,
                                     gen_targets=gen_targets,
                                     origin_targets=y_target,
                                     roll_num=2,
                                     dis_model=dis_model)
