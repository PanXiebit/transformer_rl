...
class Transformer(object):
    ...
    def build_pretrain(self, inputs, targets):
        ...
        if targets is None:
            prediction, _1, _2, _3 = self.argmax_predict(encoder_outputs, attention_bias)
            return prediction
        else:
    ...
    def build_generator(self, inputs):
        ...
        #    decoded_ids, log_probs = self.rl_predict(self.encoder_outputs, self.attention_bias)
        #    return decoded_ids, log_probs
        #else:
        tf.logging.info("!!!!!!! using argmax_predict in training/prediction/evaluation !!!!!!!!")
        decoded_ids, decoded_logits, sample_logprobs, sample_ids =  self.argmax_predict(self.encoder_outputs, self.attention_bias)
    ...
    def build_new_rollout_generator(self, gen_targets, gen_sample_ids, sample_logprobs, encoder_outputs, given_num, roll_steps=3):
        """
        ...
        def inner_loop(i, given_num, roll_gen_tgt, log_probs):
            ...
            categorical = tf.contrib.distributions.Categorical(logits=roll_logits[:, given_num, :])
        ...
        _, given_num, roll_gen_tgt, log_probs = tf.while_loop(
            cond=lambda i, given_num, _1, _2: (i < roll_steps) & (given_num < gen_tgt_len),
            body=inner_loop,
    ...
    def build_no_teacher_discriminator(self, origin_inputs, gen_target, real_loss, margin=1.0):
        ...
        _, fake_logits, _, _ = self.argmax_predict(fake_encoder_outputs, fake_attention_bias)
    ...
    def get_reward_paral_2(self, origin_inputs, gen_targets, origin_target, gen_sample_ids, sample_logprobs, roll_num, margin):
        real_loss = self.get_teach_real_loss(origin_target, origin_inputs)  # [batch]
        total_loss = 0
        ...
        batch_size = tf.shape(gen_targets)[0]
        #total_reward = tf.zeros((batch_size, 0), dtype=tf.float32)
        total_reward = []
        for i in range(roll_num):
            roll_samples, log_probs = self.build_new_rollout_generator(
                gen_targets, gen_sample_ids, sample_logprobs, self.encoder_outputs, given_num)  # [batch, tgt_len]
            print("roll_samples", roll_samples) 
            # difference between with get_reward_paral_1
            ...
            #total_loss += log_probs * tf.stop_gradient(cur_reward)
            tf.identity(log_probs[:5], "log_probs")
            total_reward.append(tf.reshape(cur_reward, (-1, 1)))
        total_reward = tf.concat(total_reward, axis=1)
        total_reward -= tf.reduce_mean(total_reward, axis=1, keepdims=True)
        total_reward /= (tf.reduce_mean(total_reward ** 2, axis=1, keepdims=True) + 1e-6)
        tf.identity(total_reward[:5], "total_reward")
        total_loss = - tf.reshape(log_probs, (-1,1)) * total_reward
        return tf.reduce_mean(total_loss) / roll_num, tf.reduce_mean(real_loss)
    ...
    def argmax_predict(self, encoder_outputs, encoder_decoder_attention_bias):
        ...
        if self.params.beam_size > 1:
            ...
            def is_not_finished(i, finished, _1, _2, _3, _4, _5, _6):
            ...
            _, _, _, decoded_ids, decoded_logits, sample_logprobs, sample_ids, _ = tf.while_loop(
                ...
                body=inner_loop,
                loop_vars=[tf.constant(0), finished, next_id, decoded_ids, decoded_logits, sample_logprobs, sample_ids, cache],
                shape_invariants=[
                    tf.TensorShape([]),
                    tf.TensorShape([None]),
                    tf.TensorShape([None, None]),
...
if __name__ == "__main__":
    ...
    # test generator
    gen_targets, decoded_logits, sample_logprobs, sample_ids = model.build_generator(x_inputs)
    ...
    #                                               margin=0.1)
    #roll_samples, log_probs = model.build_new_rollout_generator(gen_targets, logits, model.encoder_outputs, given_num=3)
    #print(log_probs)
    rewards_2 = model.get_reward_paral_2(origin_inputs=x_inputs,
                                         gen_targets=gen_targets,
                                         origin_target=y_target,
                                         gen_sample_ids=sample_ids,
