...
def get_decoder_self_attention_bias(length):
    ...
    with tf.name_scope("decoder_self_attention_bias"):
        ...
        decoder_bias = _NEG_INF * (1.0 - valid_locs)           # [1, 1, length, length]
    return decoder_bias
...
def get_padding_bias(x):
    ...
    with tf.name_scope("attention_bias"):
        ...
        attention_bias = tf.expand_dims(
            tf.expand_dims(attention_bias, axis=1), axis=1)   # [batch_size, 1, 1, length]
    return attention_bias

if __name__ == "__main__":
    tf.enable_eager_execution()
    x_inputs = tf.constant([[1,2,3,0,0],[4,5,6,6,0]], dtype=tf.int32)
    padding_mask = get_padding_bias(x_inputs)
    print(padding_mask)