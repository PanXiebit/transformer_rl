...
class Transformer(object):
    ...
    def build_pretrain(self, inputs, targets):
        with tf.variable_scope("Transformer", initializer=self._initializer, reuse=tf.AUTO_REUSE):
            ...
            if targets is None:
                tf.logging.info("!!! argmax predict !!!")
                prediction = self.argmax_predict(encoder_outputs, attention_bias)
                return prediction
            else:
                tf.logging.info("!!! decoder using teacher forcing !!!")
    ...
    def get_one_reward_baseline(self, origin_inputs, gen_targets, roll_num):
        self.baseline = self.build_bleu_discriminator(origin_inputs=origin_inputs,
                                                 gen_targets=gen_targets)
        tf.identity(self.baseline[:5], "baseline")
        ...
        tf.identity(self.total_rewards[:5], "mean_rewards")
...
if __name__ == "__main__":
    ...
    gen_targets = tf.constant([[2,3,4,5,1,1,5,2], [6,12,331,242,121,1,1,4]])
    print(gen_targets)
